...
/** GET /api/specs/:spec/proposals/:id — the edit request including its proposed YAML. */
...
/**
 * PATCH /api/specs/:spec/proposals/:id — decide an edit request.
 *
...
export async function PATCH(request: Request, { params }: Params) {
  return guard(async () => {
    ...
    if (input.action === 'withdraw') {
      ...
      const updated = await resolveProposal(spec, {
        id,
        status: 'withdrawn',
        by: viewer.login,
        note,
    ...
    if (input.action === 'reject') {
      const updated = await resolveProposal(spec, { id, status: 'rejected', by: owner.login, note });
      if (!updated) return Response.json({ error: 'It was decided already.' }, { status: 409 });
      return Response.json({ proposal: updated });
    }
    ...
    if (input.action === 'approve') {
      ...
      const merged = await mergeProposal(spec, {
        id,
        by: owner,
        note,

...
/**
 * GET  /api/specs/:spec            the published contract as YAML
 * GET  /api/specs/:spec?format=json  the same contract as JSON, for codegen
 * PUT  /api/specs/:spec            publish a new version — OWNER ONLY
 */
export async function GET(request: Request, { params }: Params) {
  ...
  const current = await getCurrentSpec(spec);
  ...
  return new Response(current.yaml, {
    headers: {
      ...
      'content-disposition': `inline; filename="${spec}.openapi.yaml"`,
    },
  });
}
...
export async function PUT(request: Request, { params }: Params) {
  return guard(async () => {
    ...
    // who is not the owner lands on POST /api/specs/:spec/proposals instead.
    ...
    const version = await saveSpecVersion(spec, {
      yaml: body.yaml,
